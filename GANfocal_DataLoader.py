...
class Train_dataset(object):
    ...
    def dataToTensor(self, iteration, blur=False):
        ...
        data_tensor = np.empty(
            [self.batch_size, self.zdepth, self.height, self.width, 1]
        )
        i = 0
        for image in data_true:
            patch = view_as_windows(
                image,
                window_shape=(((self.zdepth), (self.height), self.width)),
                step=(self.zdepth, self.height, self.width),
            )
