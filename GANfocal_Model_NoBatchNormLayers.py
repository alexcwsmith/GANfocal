...
def lrelu1(x):
    return tf.maximum(x, 0.15 * x)

def lrelu2(x):
    return tf.maximum(x, 0.25 * x)

def prelu(x, dtype, partition_info=None):
    alpha=tf.Variable(0.15, dtype='float32')
    return tf.math.maximum(tf.cast(x,tf.float32), x*alpha)
...
=        x = GlobalMeanPool3d(x, name="GAP3d")
...
def train(
    ...
    decay_rate = 0.5  # was 0,5
    decay_steps = 750  # every 2 epochs (more or less)
    ...
    for j in range(val_restore, epochs + val_restore):
        random.shuffle(traindata.file_list)
