...
def discriminator(input_disc, kernel, img_zdepth, img_width, img_height, batch_size=1, reuse=None, is_train=True):
    w_init = tf.random_normal_initializer(stddev=0.02)
    with tf.compat.v1.variable_scope("SRGAN_d", reuse=reuse):
#        tl.layers.set_name_reuse(reuse)
        input_disc.set_shape([batch_size, img_zdepth, img_height, img_width,  1], ) #switched width height
        ...
        x = BatchNormLayer(x, decay=0.9, is_train=is_train, name='BN1-conv2', act=lrelu2)
...
def generator(input_gen, kernel, feature_size, img_zdepth, img_height, img_width,  
              subpixel_NN=True, nn=True, upscaling_factor=2, num_blocks=6, reuse=None, is_train=True):
    w_init = tf.random_normal_initializer(stddev=0.02)
    ...
    with tf.compat.v1.variable_scope("SRGAN_g", reuse=reuse):
        ...
        if subpixel_NN:
            ...
            x = DeConv3dLayer(x, shape=(kernel * 2, kernel * 2, kernel * 2, 64, feature_size),
                              ...
                              output_shape=(tf.shape(input_gen)[0], img_zdepth_deconv, img_height_deconv, img_width_deconv,
                                             64),
            ...
            if upscaling_factor == 4:
                x = DeConv3dLayer(x, shape=(kernel * 2, kernel * 2, kernel * 2, 64, 64),
                                  ...
                                  output_shape=(tf.shape(input_gen)[0], img_zdepth, img_height, img_width, 64),
        ...
        # ____________SUBPIXEL - BASELINE______________#
            ...
            x = Conv3dLayer(x, shape=(kernel, kernel, kernel, feature_size, 64), act=lrelu1,
                            strides=(1, 1, 1, 1, 1,),
                            padding='SAME', W_init=w_init, name='conv1-ub/1')
            arguments = {'img_zdepth': img_zdepth, 'img_height': img_height, 'img_width': img_width,  
            ...
            if upscaling_factor == 4:
                x = Conv3dLayer(x, shape=(kernel, kernel, kernel, int((64) / 8), 64), act=lrelu1,
                                strides=(1, 1, 1, 1, 1,),
                                padding='SAME', W_init=w_init, name='conv1-ub/2')
                arguments = {'img_zdepth': img_zdepth, 'img_height': img_height, 'img_width': img_width, 'stepsToEnd': 1,
                             'n_out_channel': int(64 / 8)}
...
def train(modelName, img_zdepth, img_width, img_height, kernel, img_path=os.getcwd(), result_dir=os.getcwd(), checkpoint_dir=os.getcwd(), upscaling_factor=2, residual_blocks=6, feature_size=64,
           subpixel_NN=True, nn=False, restore=None, batch_size=1, epochs=10, saveiters=9, train_fraction=0.8):
    traindata = Train_dataset(data_path=img_path, zdepth=img_zdepth, height=img_height, width=img_width, batch_size=batch_size, train_portion=train_fraction)
    
    iterations_train = math.ceil((len(traindata.file_list)) / batch_size)

    # ##========================== DEFINE MODEL ============================##
    t_input_gen = tf.compat.v1.placeholder('float32', [int(batch_size), img_zdepth/2, img_height/2, img_width/2, 1],
                                 name='t_image_input_to_SRGAN_generator')
    
    t_target_image = tf.compat.v1.placeholder('float32', [int(batch_size), img_zdepth, img_height, img_width,   1],
                                                              name='t_target_image')
    
   # t_input_mask = tf.compat.v1.placeholder('float32', [int(batch_size), img_zdepth, img_height, img_width,  1],
   #                               name='t_image_input_mask')
    


    net_gen = generator(input_gen=t_input_gen, kernel=kernel, num_blocks=residual_blocks, upscaling_factor=upscaling_factor,
                        img_zdepth=traindata.zdepth, img_height=traindata.height, img_width=traindata.width,
                        subpixel_NN=subpixel_NN, nn=nn,
                        feature_size=feature_size, is_train=True, reuse=False)
    net_d, disc_out_real = discriminator(input_disc=t_target_image, img_zdepth=traindata.zdepth, 
                        img_height=traindata.height, img_width=traindata.width,  
                        kernel=kernel, is_train=True, reuse=False)
    _, disc_out_fake = discriminator(input_disc=net_gen.outputs, img_zdepth=traindata.zdepth, img_height=traindata.height, 
                        img_width=traindata.width,  
                        kernel=kernel, is_train=True, reuse=True)

    # test
    gen_test = generator(t_input_gen, kernel, feature_size, img_zdepth, img_height, img_width,
                         num_blocks=residual_blocks, upscaling_factor=upscaling_factor, subpixel_NN=subpixel_NN, nn=nn,  is_train=True, reuse=True)
...
#g_gan loss was 10e-2
    g_gan_loss = 10e-3 * tf.reduce_mean(tf.square(disc_out_fake - smooth_gan_labels(tf.ones_like(disc_out_real))),
                                        name='g_loss_gan')
    ...
    learning_rate = tf.compat.v1.train.inverse_time_decay(lr_v, global_step=global_step, decay_rate=decay_rate,
                                                decay_steps=decay_steps)

    # Optimizers
    g_optim = tf.compat.v1.train.AdamOptimizer(learning_rate).minimize(g_loss, var_list=g_vars)
    d_optim = tf.compat.v1.train.AdamOptimizer(learning_rate).minimize(d_loss, var_list=d_vars)
...
#x_generator is now lr_generator
            ...
            # ========================= train SRGAN ========================= #
            # update D
            errd, _ = session.run([d_loss, d_optim], {t_target_image: hr, t_input_gen: lrgenin}, options=run_opts) #added options
            # update G
            errg, errmse, errgan, errgd, _ = session.run([g_loss, mse_loss, g_gan_loss, gd_loss, g_optim],
                                                         {t_input_gen: lrgenin, t_target_image: hr}) #deleted t_input_mask: xm
            print(
                "Epoch [%2d/%2d] [%4d/%4d]: d_loss: %.8f g_loss: %.8f (mse: %.6f gdl: %.6f adv: %.6f)" % (
                    j, epochs + val_restore, i, iterations_train, errd, errg, errmse, errgd,
                    errgan))
            with open(os.path.join(result_dir, 'losses_' + modelName + '.txt'), 'a+') as log:
                log.write("Epoch [%2d/%2d] [%4d/%4d]: d_loss: %.8f g_loss: %.8f (mse: %.6f gdl: %.6f adv: %.6f)" % (
                    j, epochs + val_restore, i, iterations_train, errd, errg, errmse, errgd,
                    errgan) + "\n")
                log.close()
            k = k+1
    ...
    #            if j - val_restore == 0:
     ...
     #       img_true = nib.Nifti1Image(x_true_img, np.eye(4))
            if step % saveiters == 0:
                io.imsave(os.path.join(result_dir, str(f) + '_Epoch' + str(step) + '_hr_true.tif'), hr_true_img)
            lr_gen_img = lrgenin[0]
            if normfactor != 0:
                lr_gen_img = ((lr_gen_img + 1) * normfactor)  # denormalize
       #     img_gen = nib.Nifti1Image(x_gen_img, np.eye(4))
            if step % saveiters == 0:
                io.imsave(os.path.join(result_dir, str(f) + '_Epoch' + str(step) + '_lr_gen.tif'), lr_gen_img)
         ...
         #   lr_img_pred = nib.Nifti1Image(x_pred_img, np.eye(4))
            if step % saveiters ==0:
                io.imsave(os.path.join(result_dir, str(f) + '_Epoch' + str(step) + '_lr_pred.tif'), lr_pred_img)
            ...
            val_ssim = ssim(hr_true_img, lr_pred_img, data_range=val_max - val_min, multichannel=True)
...
def evaluate(modelName, kernel, img_zdepth, img_height, img_width, img_path, checkpoint_dir, result_dir,
             batch_size=1, feature_size=64, upscaling_factor=2, residual_blocks=6, subpixel_NN=True, nn=False, 
             ):
    testdata = Test_dataset(data_path=img_path, zdepth=img_zdepth, height=img_height, width=img_width, batch_size=batch_size, test_portion=0.2)
    ...
    srgan_network = generator(input_gen=t_input_gen, kernel=kernel, num_blocks=residual_blocks, upscaling_factor=upscaling_factor,
                        img_zdepth=testdata.zdepth, img_height=testdata.height, img_width=testdata.width,
                        subpixel_NN=subpixel_NN, nn=nn,
                        feature_size=feature_size, is_train=False, reuse=False)
    ...
    #    lr_generator = gaussian_filter(lr_generator, sigma=1)
        ...
        sr_generated = sess.run(srgan_network.outputs, {t_input_gen: lr_generator[np.newaxis, :]})
   ...
   #     volume_mask = aggregate(xt_mask)
        ...
        totalpsnr += val_psnr
        val_ssim = ssim(volume_real, volume_generated,
        ...
        print(val_ssim)
        # save volumes
        filename_gen = os.path.join(result_dir, str(f) + '_' + str(i) + '_' + str(modelName) + 'gen.tif')
        img_volume_gen = io.imsave(filename_gen, volume_generated)
        filename_real = os.path.join(result_dir, str(f) + '_' + str(i) + '_' + str(modelName) + 'real.tif')
        img_volume_real = io.imsave(filename_real, volume_real)

    with open(os.path.join(result_dir, modelName + '_EvaluationResults.txt'), 'a+') as log:
        log.write(
